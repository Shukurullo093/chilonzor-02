...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function groupDataByCrimeType(data) {
        const grouped = {};
        data.forEach(item => {
    ...
    function updateStats(data) {
        ...
        const crimeTypes = new Set(data.map(item => item.crime_type)).size;
        const neighborhoods = new Set(data.map(item => item.neighborhood)).size;

        document.getElementById('totalCrimes').textContent = totalCrimes;
        document.getElementById('crimeTypes').textContent = crimeTypes;
        document.getElementById('neighborhoods').textContent = neighborhoods;
    }
    ...
    function aiAdvice(targetCrimeType) {        
        ...
        function generatePredictions() {
            ...
            const predictions = [
                {
                    district: "Чилонзор",
                    crimeType: "Наркотик моддалар билан боглик жиноятлар",
                    riskLevel: "Юқори",
                    timeWindow: "Кечқурун (18:00-24:00)"
                },
                {
                    district: "Бектопи",
                    crimeType: "Ўғирлик",
                    riskLevel: "Ўрта",
                    timeWindow: "Кундузи (12:00-18:00)"
                },
                {
                    district: "Қатортол",
                    crimeType: "Наркотик моддалар билан боглик жиноятлар",
                    riskLevel: "Ўрта",
                    timeWindow: "Тун (00:00-06:00)"
                },
                {
                    district: "Тирсақобод",
                    crimeType: "Ўғирлик",
                    riskLevel: "Паст",
                    timeWindow: "Эрталаб (06:00-12:00)"
                }
            ];
        ...
        const firstTab = new bootstrap.Tab(document.getElementById('predictions-tab'));
        firstTab.show();
    }
    ...
    function generatePredictions() {
        ...
        const predictions = [
            {
                ...
                riskLevel: "critical",
                timeWindow: "Кечқурун (22:00-24:00)",
                trend: "↑ 15%",
                prevention: {
                    title: "Қотиллик жиноятларини олдини олиш",
                    measures: [
                        "Қўшимча патрулларни жойлаштириш",
                        "Кузатув камераларини кўпайтириш",
                        "Маҳалла фаоллари билан йиғилишлар ўтказиш",
                        "Хавфли жойларда нурлантиришни яхшилаш"
                    ]
                }
            },
            {
                ...
                trend: "↑ 12%",
                prevention: {
                    title: "Наркотик жиноятларига қарши курашиш",
                    measures: [
                        "Наркоконтроль ходимларини жойлаштириш",
                        "Ёшлар орасида профилактик ишлар",
                        "Шубҳали жойларга рейдлар",
                        "Аноним хабар бериш тизимини жорий этиш"
                    ]
                }
            },
            {
                ...
                trend: "→ 0%",
                prevention: {
                    title: "Ўғирликка қарши чоралар",
                    measures: [
                        "Дўконлар хавфсизлигини текшириш",
                        "Харидорларга эхтиёт бўлиш ҳақида эслатмалар",
                        "Қўриқловчиларни кўпайтириш",
                        "Қимматбаҳо буюмларни хавфсиз жойда сақлаш бўйича маълумот бериш"
                    ]
                }
            },
            {
                district: "Тирсақобод",
                crimeType: "Безорилик",
                crimeCategory: "public",
                riskLevel: "low",
                timeWindow: "Эрталаб (06:00-12:00)",
                trend: "↓ 8%",
                prevention: {
                    title: "Безориликни камайтириш",
                    measures: [
                        "Оилалар билан ишлаш",
                        "Ичкиликка мойил шахсларни қайд этиш",
                        "Маҳалла йиғилишларини кўпайтириш",
                        "Жамoat тартиби қоидалари ҳақида огоҳлантириш"
                    ]
                }
            }
        ];
        ...
        predictions.forEach(prediction => {
            ...
            let trendClass = '';
            let trendIcon = '';
            if (prediction.trend.includes('↑')) {
                trendClass = 'trend-up';
                trendIcon = '<i class="bi bi-arrow-up-short trend-icon"></i>';
            } else if (prediction.trend.includes('↓')) {
                trendClass = 'trend-down';
                trendIcon = '<i class="bi bi-arrow-down-short trend-icon"></i>';
            } else {
                trendClass = 'trend-neutral';
                trendIcon = '<i class="bi bi-dash-lg trend-icon"></i>';
            }
            
            html += `
                <tr>
                    <td><strong>${prediction.district}</strong></td>
                    <td><span class="crime-tag crime-${prediction.crimeCategory}">${prediction.crimeType}</span></td>
                    <td><span class="risk-badge ${riskClass}">${riskText}</span></td>
                    <td>${prediction.timeWindow}</td>
                    <td><span class="trend-indicator ${trendClass}">${trendIcon}${prediction.trend}</span></td>
                </tr>
            `;
        });
        ...
        generatePreventionMeasures(predictions);
        
        // Generate time analysis
        generateTimeAnalysis();
    }

    function generatePreventionMeasures(predictions) {
        const preventionTab = document.getElementById('prevention');
        
        let preventionHtml = `
            <div class="prevention-section">
                <h4 class="text-center mb-4" style="color: #4a90e2;">Жиноятларни олдини олиш бўйича таклифлар</h4>
        `;
            ...
            preventionHtml += `
                <div class="prevention-card">
                    <div class="prevention-card-header">
                        <div class="prevention-card-icon">
                            <i class="bi bi-shield-check"></i>
                        </div>
                        <h5 class="prevention-card-title">${prediction.prevention.title}</h5>
                    </div>
                    <ul class="prevention-list">
                        ${prediction.prevention.measures.map(measure => `
                            <li class="prevention-item">
                                <i class="bi bi-check-circle-fill prevention-item-icon"></i>
                                <span>${measure}</span>
                            </li>
                        `).join('')}
                    </ul>
        ...
        preventionHtml += `
            <div class="prevention-card">
                <div class="prevention-card-header">
                    ...
                    <h5 class="prevention-card-title">Умумий олдини олиш чоралари</h5>
                </div>
                <ul class="prevention-list">
                    <li class="prevention-item">
                        <i class="bi bi-check-circle-fill prevention-item-icon"></i>
                        <span>Маҳаллийлаштирилган политсия хизмати дастурларини жорий этиш</span>
                    </li>
                    <li class="prevention-item">
                        <i class="bi bi-check-circle-fill prevention-item-icon"></i>
                        <span>Фуқаролар жиноятлар ҳақида аноним хабар бериш учун мобил иловаларни ишлаб чиқиш</span>
                    </li>
                    <li class="prevention-item">
                        <i class="bi bi-check-circle-fill prevention-item-icon"></i>
                        <span>Жиноят кўп содир бўладиган жойларга қўшимча кузатув камераларини ўрнатиш</span>
                    </li>
                    <li class="prevention-item">
                        <i class="bi bi-check-circle-fill prevention-item-icon"></i>
                        <span>Хавфсизлик муаммолари ва уларнинг ечимлари муҳокамаси учун мунтазам маҳалла йиғилишларини ўтказиш</span>
                    </li>
                </ul>
            </div>
        `;
        ...
        preventionHtml += `</div>`;
        preventionTab.innerHTML = preventionHtml;
    }

    // Crime Analysis Modal Functions
    function showCrimeModal(crimeType) {
        // Update modal content based on selected crime type
        updateModalContent(crimeType);
    ...
    function updateModalContent(crimeType) {
        // Set modal title with crime type
        document.getElementById('legendModalLabel').innerHTML = `
            <i class="bi bi-graph-up me-2"></i>${crimeType} диагностикаси
        `;
        
        // Analyze crime data
        const crimeData = analyzeCrimeData(crimeType);
        
        // Render charts
        renderCrimeTrendChart(crimeData);
        renderTimePatternChart(crimeData);
        renderNeighborhoodChart(crimeData);
    }

    function analyzeCrimeData(crimeType) {
        // Filter data for selected crime type
        const filteredData = data3.filter(item => item.crime_type === crimeType);
        ...
        filteredData.forEach(item => {
            ...
            if (item.time) {
                ...
                if (hour >= 0 && hour < 6) timePeriods["00:00-06:00"]++;
                else if (hour >= 6 && hour < 12) timePeriods["06:00-12:00"]++;
                else if (hour >= 12 && hour < 18) timePeriods["12:00-18:00"]++;
                else if (hour >= 18 && hour < 24) timePeriods["18:00-24:00"]++;
            }
            
            // Analyze neighborhoods
            if (item.neighborhood) {
                neighborhoodCounts[item.neighborhood] = (neighborhoodCounts[item.neighborhood] || 0) + 1;
            }
        ...
        Object.keys(timePeriods).forEach(key => {
            timePercentages[key] = totalCrimes > 0 ? Math.round((timePeriods[key] / totalCrimes) * 100) : 0;
        });
        
        const neighborhoodPercentages = {};
        Object.keys(neighborhoodCounts).forEach(key => {
            neighborhoodPercentages[key] = totalCrimes > 0 ? Math.round((neighborhoodCounts[key] / totalCrimes) * 100) : 0;
        });
        ...
        let maxCount = 0;
        Object.keys(timePeriods).forEach(key => {
            if (timePeriods[key] > maxCount) {
                maxCount = timePeriods[key];
                peakTime = key;
            }
        });
        
        // Find top neighborhoods (top 5)
        ...
        if (totalCrimes > 50) {
            riskLevel = "critical";
            riskPercentage = 85;
        } else if (totalCrimes > 30) {
            riskLevel = "high";
            riskPercentage = 65;
        } else if (totalCrimes > 15) {
            riskLevel = "medium";
            riskPercentage = 45;
        } else {
            riskLevel = "low";
    ...
    function renderCrimeTrendChart(crimeData) {
        // Prepare month data
        // const months = months;
        const monthData = months.map(month => crimeData.monthCounts[month] || 0);
        
        const options = {
            series: [{
                name: crimeData.crimeType,
                data: monthData
            }],
            chart: {
                type: 'line',
    ...
    function renderTimePatternChart(crimeData) {
        const timeCategories = Object.keys(crimeData.timePeriods);
        const timeData = timeCategories.map(key => crimeData.timePeriods[key]);
        ...
        const options = {
            ...
            tooltip: {
                ...
                y: {
                    formatter: function(value) {
                        return value + " та (" + Math.round((value / crimeData.totalCrimes) * 100) + "%)";
                    }
                }
            },
            grid: {
    ...
    function renderNeighborhoodChart(crimeData) {
        // Prepare neighborhood data (top 5)
        const neighborhoods = crimeData.topNeighborhoods.map(item => item[0]);
        const neighborhoodData = crimeData.topNeighborhoods.map(item => item[1]);
        ...
        const chart = new ApexCharts(document.querySelector("#neighborhoodCrimeChart"), options);
        chart.render();
    }

    // Crime Prediction Functions
    function generateCrimePredictions() {
        // In a real system, this would come from a predictive model
        // For demo purposes, we'll generate sample predictions
        ...
        const predictions = [
            {
                ...
                riskLevel: "high",
                location: [41.2858, 69.2043] // Sample coordinates
            },
            {
                neighborhood: "Ал-Хоразмий",
                day: "Шанба",
                time: "12:00-18:00",
                crimeType: "Наркотик моддалар билан боглик жиноятлар",
                probability: 65,
                expectedCount: "1-2 та",
                recentCases: "8 та (соңги 4 хафтада)",
                riskLevel: "medium",
                location: [41.2923, 69.1967]
            },
            {
    ...
    function renderPredictions(predictions, filters = {}) {
        const container = document.querySelector('.prediction-grid');
        container.innerHTML = '';
        
        // Apply filters
        let filteredPredictions = predictions.filter(pred => {
            if (filters.neighborhood && filters.neighborhood !== 'all' && pred.neighborhood !== filters.neighborhood) {
                return false;
            }
            if (filters.day && filters.day !== 'all' && pred.day !== filters.day) {
                return false;
            }
            if (filters.time && filters.time !== 'all' && pred.time !== filters.time) {
                return false;
            }
            return true;
        });
        ...
        filteredPredictions.forEach(pred => {
            ...
            card.className = `prediction-card prediction-${pred.riskLevel}`;
            
            card.innerHTML = `
                <div class="prediction-card-header">
                    <div class="prediction-risk">${pred.riskLevel === 'high' ? 'ЮҚОРИ ХАВФ' : pred.riskLevel === 'medium' ? 'ЎРТА ХАВФ' : 'ПАСТ ХАВФ'}</div>
                ...
                <div class="prediction-content">
                    ...
                    <div class="prediction-stats">
                        <div class="prediction-stat">
                            ...
                            <span class="stat-value">${pred.probability}%</span>
                        </div>
                        <div class="prediction-stat">
                            <span class="stat-label">Сонӣ бошорат:</span>
                            <span class="stat-value">${pred.expectedCount}</span>
                        </div>
                        <div class="prediction-stat">
                            <span class="stat-label">Қайд этилган жиноятлар:</span>
                            <span class="stat-value">${pred.recentCases}</span>
                        </div>
                    </div>
                </div>
            `;
        ...
        updatePredictionMap(filteredPredictions);
    }

    function updatePredictionMap(predictions) {
        // Initialize map if not already initialized
        if (!window.predictionMap) {
            window.predictionMap = new ymaps.Map('predictionMap', {
                center: [41.2858, 69.2043], // Chilonzor approximate center
                zoom: 13,
                controls: ['zoomControl']
            });
            
            // Add district polygons (in real app these would be accurate shapes)
            const districts = {
                'Қозиробод': [[41.288, 69.201], [41.288, 69.207], [41.283, 69.207], [41.283, 69.201]],
                'Ал-Хоразмий': [[41.292, 69.193], [41.292, 69.199], [41.287, 69.199], [41.287, 69.193]],
                'Бешёғоч': [[41.291, 69.205], [41.291, 69.211], [41.286, 69.211], [41.286, 69.205]],
                'Меҳржон': [[41.283, 69.196], [41.283, 69.202], [41.278, 69.202], [41.278, 69.196]]
            };
            ...
            Object.entries(districts).forEach(([name, coordinates]) => {
                ...
                window.predictionMap.geoObjects.add(polygon);
            });
        }
        
        // Clear existing placemarks
        // window.predictionMap.geoObjects.removeAll();
        
        // Add new placemarks for predictions
        predictions.forEach(pred => {
            ...
            const placemark = new ymaps.Placemark(pred.location, {
                hintContent: `${pred.neighborhood}: ${pred.crimeType}`,
                balloonContent: `
                    <div style="padding: 10px;">
                        <h4 style="margin: 0 0 10px 0; color: ${color};">${pred.crimeType}</h4>
                        <p><strong>Маҳалла:</strong> ${pred.neighborhood}</p>
                        <p><strong>Кун:</strong> ${pred.day}</p>
                        <p><strong>Вақт:</strong> ${pred.time}</p>
                        <p><strong>Эҳтимоллик:</strong> ${pred.probability}%</p>
                        <p><strong>Сонӣ бошорат:</strong> ${pred.expectedCount}</p>
                    </div>
                `
            }, {
                iconLayout: 'default#image',
                iconImageHref: 'https://cdn.jsdelivr.net/npm/leaflet@1.7.1/dist/images/marker-icon.png',
                iconImageSize: [25, 41],
                iconImageOffset: [-12, -41]
            });
            
            // window.predictionMap.geoObjects.add(placemark);
        });
    }

    // Initialize prediction tab
    function initPredictionTab() {
        const predictions = generateCrimePredictions();
        renderPredictions(predictions);
        
        // Setup filter event listeners
        document.getElementById('predictionNeighborhood').addEventListener('change', function() {
            const filters = {
                neighborhood: this.value,
                day: document.getElementById('predictionDay').value,
                time: document.getElementById('predictionTime').value
            };
            renderPredictions(predictions, filters);
        });
        
        document.getElementById('predictionDay').addEventListener('change', function() {
            const filters = {
                neighborhood: document.getElementById('predictionNeighborhood').value,
                day: this.value,
                time: document.getElementById('predictionTime').value
            };
            renderPredictions(predictions, filters);
        ...
        // Initialize map when Yandex Maps API is loaded
        if (typeof ymaps !== 'undefined') {
            updatePredictionMap(predictions);
        } else {
            // In case Yandex Maps isn't loaded yet
...
// noma'lum - 1
// ijtimoiy xavfi kam bo'lgan - 2,
// o'rta - 3,
// og'ir - 4,
//  o'ta og'ir - 5

const neighborhoodsColor = {};

data3.forEach(item => {
  const nbh = item.neighborhood;
  const severity = item.crimeSeverity; 
    
  if (!neighborhoodsColor[nbh]) {
    neighborhoodsColor[nbh] = {
      totalCrimes: 0,
      totalSeverity: 0,
      avgSeverity: 0
    };
  ...
  neighborhoodsColor[nbh].totalCrimes += 1;
  neighborhoodsColor[nbh].totalSeverity += parseInt(severity);
  neighborhoodsColor[nbh].avgSeverity = (neighborhoodsColor[nbh].totalSeverity / neighborhoodsColor[nbh].totalCrimes).toFixed(2);
});

const avgValues = Object.values(neighborhoodsColor).map(d => parseFloat(d.avgSeverity));
...
// 2. Rang ajratamiz
Object.entries(neighborhoodsColor).forEach(([name, stats]) => {
  const val = parseFloat(stats.avgSeverity);
  
  if (val < min + step) {
    stats.level = "yashil";
  } else if (val < min + 2 * step) {
    stats.level = "sariq";
  } else {
    stats.level = "qizil";
  }
...
for (const [key, value] of Object.entries(neighborhoodsColor)) {
  const rating = value.level;
  if (grouped[rating]) {
    grouped[rating].push(key);  // yoki key
  } else {
    console.warn(`Nomaʼlum reyting: ${key} -> ${rating}`);
  }
}
...
Object.entries(grouped).forEach(([rating, neighborhoods]) => {
    const card = document.createElement("div");
    card.className = `card ${rating}`;

    const header = document.createElement("h3");
    header.textContent = `${neighborhoods.length}`;
...
ymaps.ready(init);
function init() {
    var map = new ymaps.Map("predictionMap", {
    ...
    for (var key in neighborhoods) {
        districtPolygons.push(neighborhoods[key]);
    ...
    function createDistrictPolygon(coordinates, name, area) {
        ...
        polygon.events.add('click', () => {
            ...
            // Sadece setBounds kullanmak yeterli olabilir
            ...
            if (typeof name !== 'undefined') {
                ...
                console.error('Name variable is not defined');
            }
        });
    ...
    // Har bir poligon uchun rangni yangilaymiz
    districtPolygons.forEach(dp => {  
        let color = null;

        Object.entries(neighborhoodsColor).forEach(([key, value]) => {          
            if (normalizeString(key).includes(normalizeString(dp.name))) {
                color = value.level;
            }
        });
        
        const count = color ? color : 0;
        color = count === 0 ? '#e0e0e0' : getColor(color);
        
        dp.polygon.options.set('fillColor', color);
