...
function filterByDistrict(districtName) {
    const filtered = data3.filter(item => item.neighborhood === districtName);

    if (filtered.length > 0) {
        renderChart(filtered);
    } else {
        console.warn(`"${districtName}" uchun ma'lumot topilmadi`);
    }
}
...
function updateMapColors(selectedCrimeType) {    
    ...
    data3.forEach(item => {
        if (item.crime_type === selectedCrimeType) {
            ...
            if (!countsByDistrict[district]) {
                countsByDistrict[district] = 0;
            }
            countsByDistrict[district]++;
        }
    });
...
function resetMapColors() {
    districtPolygons.forEach(dp => {
        dp.polygon.options.set('fillColor', '#AAAAAA');
    });
}
...
function normalizeString(str) {
    const map = {
        ...
        'Т': 'T', 'т': 't', 'У': 'U', 'у': 'u', 'Ў': 'O‘', 'ў': 'o‘', 'Ф': 'F', 'ф': 'f',
        'Х': 'X', 'х': 'x', 'Ц': 'Ts', 'ц': 'ts', 'Ч': 'Ch', 'ч': 'ch', 'Ш': 'Sh', 'ш': 'sh',
        'Щ': 'Sh', 'щ': 'sh', 'Ъ': '', 'ъ': '', 'Ы': 'I', 'ы': 'i', 'Ь': '', 'ь': '',
        'Э': 'E', 'э': 'e', 'Ю': 'Yu', 'ю': 'yu', 'Я': 'Ya', 'я': 'ya', 'Ғ': 'G‘', 'ғ': 'g‘',
        'Ҳ': 'H', 'ҳ': 'h'
    };
    return str
        .split('')
